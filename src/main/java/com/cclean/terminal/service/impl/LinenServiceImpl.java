...
public class LinenServiceImpl implements LinenService {
    ...
    public Result pack(String accessToken, RfidsVO rfidsVO) throws BusinessException {
        ...
        jsonParam.put("skuId", skuId);
        jsonParam.put("count", skuStatistics.getCount());
        jsonParam.put("rfids", rfidset);
        String datajson = InvokeUtil.invokeString(url, accessToken, jsonParam);
        TLinenPack tLinenPack = JSONObject.parseObject(datajson, TLinenPack.class);
    ...
    @Override
    public Result types(String accessToken) throws BusinessException {
        Result result = Result.success();
        if (accessToken == null) return Result.paramNull();
